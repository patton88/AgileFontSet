...
extern int CStringSplitPath(vector<CString>& vecResult, const CString& str, CString strSep);	//返回字段数
...
// 若 strFile 是相对路径，便添加当前目录。并替换转义字符
CString& addCurDir(CString& strFile)
{
	CTrimQ(strFile);

	if (L'\"' == strFile[0]) {
		strFile.Replace(L"\"", L"");		//正确做法：删除str中的双引号
		CTrimQ(strFile);
		if (L':' != strFile[1]) {	//判断Path第2个字符非L':'，便非绝对路径，加上m_strCurrentDir
			strFile = getCurDir(2) + strFile;
		}
		strFile = L"\"" + strFile + L"\"";	//处理后再加上双引号
	}
	else
	{
		if (L':' != strFile[1]) {	//判断Path第2个字符非L':'，便非绝对路径，加上m_strCurrentDir
...
";
	...
	iSeg = CStringSplitPath(vecCmdLine, strCmdLine, ' ');	//strCmdLine中的参数段可不分先后次序
	...
	{
		CTrimQ(str);		//去除str两端空格

		if (L".ini" == StrToLower(str.Right(4)))		//CStringSplitPath()分割后已不含双引号
		{	//当有两个L".ini"参数时，只用前面一个参数
			if (vecStrCmd[0].IsEmpty()) {
				vecStrCmd[0] = addCurDir(str);	//检测并添加当前目录
			}
		...
		else if (L'-' == str[0] && isLegal(str) > 0) {	//当有两个L"-xxx"参数时，只用前面一个参数
			// xxx 可以是：Win8xPreset、Win10Preset、UserPreset1 - UserPreset100 之一
			str = str.Right(str.GetLength() - 1);	//去掉前面的L"-"
	...
	// 处理有path：参数为1段path、或者2段path -hide、或者3段path -xxx -hide
		...
		//int x = _waccess(strT1, 0);	//_waccess用的path要去掉双引号
		...
		//处理参数为2段path -hide
			...
			progsheet.m_pp1FontSet.OnSet(0, 0xFFFF, NULL, nCmdShow);	//后台处理标志 0xFFFF
		}
